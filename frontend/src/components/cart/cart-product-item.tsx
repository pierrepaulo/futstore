...
export const CartProductItem = ({ item }: Props) => {
  ...
  const handleRemove = async () => {
    cartStore.removeItem(item.product.id, item.size);
    await updateCookie();
  };
    ...
    <div className="flex items-center p-6 gap-4 md:gap-8 border-0 md:border-b border-gray-200">
      <div className="border border-gray-200 p-1">
        {imageSrc ? (
          ...
          <div className="size-24 flex items-center justify-center bg-gray-50 text-[10px] text-gray-400">
            Sem imagem
          </div>
        )}
      </div>
      <div className="flex-1 flex flex-col md:flex-row justify-between md:items-center">
        <div>
          ...
          <div className="text-xs text-gray-500 mb-1">Tamanho: {item.size}</div>
